...
class axi_i2c_driver extends uvm_driver #(axi_i2c_seq_item);
    ...
    task axi_write(input [3:0] addr, input [31:0] data);
        ...
        vif.wdata  <= data; vif.wstrb   <= 4'hF; vif.wvalid <= 1;
    ...
    // ✅ [결정적 수정] 이중 실행을 막기 위한 펄스 형태의 명령어 인가
    task execute_cmd(input [4:0] cmd, input [7:0] tx_val = 0, input bit has_tx = 0);
        logic [31:0] rdata;
    ...
    virtual task run_phase(uvm_phase phase);
        ...
        forever begin
            ...
            execute_cmd(5'b00010, item.tx_data_m, 1);         // 3. WRITE: 데이터 페이로드 (has_tx=1)
   ...
   // ----------------------------------------------------
            ...
            execute_cmd(5'b00010, 8'h25, 1);                  // 6. WRITE: Slave Addr + Read 모드
            
            //  [핵심 수정] 5'b00100 -> 5'b10100 (Bit 4를 1로 설정하여 NACK 전송)
            execute_cmd(5'b10100);                            // 7. READ : 데이터 수신 후 슬레이브에게 NACK 전송
    ...
    virtual task run_phase(uvm_phase phase);
        ...
        forever begin
            ...
            if (vif.rvalid && vif.rready && vif.araddr == 4'h8) begin
                axi_i2c_seq_item item = axi_i2c_seq_item::type_id::create("item");
...
class axi_i2c_scoreboard extends uvm_scoreboard;
    ...
    function void write(axi_i2c_seq_item item);
        ...
        `uvm_info("SCB_DATA_CHECK", $sformatf(
                  ...
                  item.tx_data_m, item.rx_data_s, tx_match ? "PASS" : "FAIL",
                  item.tx_data_s, item.rx_data_m, rx_match ? "PASS" : "FAIL"
                  ), UVM_MEDIUM)
        ...
        if (tx_match && rx_match) begin
            ...
            fail_cnt++;
            `uvm_error("SCB_MISMATCH", "데이터 불일치가 발생하였습니다.")
        end
    endfunction

    virtual function void report_phase(uvm_phase phase);
        `uvm_info("SCB_FINAL", "\n\n================ 전체 검증 요약 ================", UVM_LOW)
        `uvm_info("SCB_FINAL", $sformatf("  총 테스트 횟수 : %0d 회", pass_cnt + fail_cnt), UVM_LOW)
        `uvm_info("SCB_FINAL", $sformatf("  성공(MATCH)    : %0d 회", pass_cnt), UVM_LOW)
        `uvm_info("SCB_FINAL", $sformatf("  실패(MISMATCH) : %0d 회", fail_cnt), UVM_LOW)
        `uvm_info("SCB_FINAL", "================================================\n", UVM_LOW)
    endfunction
endclass
...
class axi_i2c_coverage extends uvm_subscriber #(axi_i2c_seq_item);
    ...
    virtual function void report_phase(uvm_phase phase);
        ...
        `uvm_info("COV", $sformatf(" Total Coverage = %.1f%%", cg_data.get_coverage()), UVM_LOW)
        `uvm_info("COV", $sformatf(" TX Data (M) Coverage = %.1f%%", cg_data.cp_tx_data_m.get_coverage()), UVM_LOW)
        `uvm_info("COV", $sformatf(" TX Data (S) Coverage = %.1f%%", cg_data.cp_tx_data_s.get_coverage()), UVM_LOW)
        `uvm_info("COV", " =========================== \n\n", UVM_LOW)
    endfunction
endclass
